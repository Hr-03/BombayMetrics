...
let directId=sessionStorage.getItem("rootFolder");
...
<Sidebar/>
    ...
    <Row className='m-4'>
      <Col>
      <Card className='p-3'>
        <p className='docTitle mb-4'>{`Your Documents > ${dirName}`}</p>
        <Row>
          <Col>
          <Row>
<Col md={8}>
      ...
      {!toggleView &&  <Row className='mt-4'>
          ...
          <Card className='foldCrd p-3 border-0 m-2' onClick={()=>{
            sessionStorage.setItem("insideFolder",dir?.DirectoryID)
sessionStorage.setItem("lvl1Name",dir?.DirectoryName)
  ...
  toggleView && directories?.map((drc,i)=>{
    return(
      ...
      <Card className='foldCrd p-3 pt-0 border-0 m-2' onClick={()=>{
            ...
            sessionStorage.setItem("lvl1Name",drc?.DirectoryName)
